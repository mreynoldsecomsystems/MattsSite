...
var nicEditorConfig = bkClass.extend({
	...
	iconsPath : '../Content/nicEditorIcons.gif',
...
var nicEditorButton = bkClass.extend({
	...
	construct : function(e,buttonName,options,nicEditor) {
		...
		if (buttonName == 'bgcolor') {
		    specialClass = "colorPicker";
		    this.button = new bkElement('DIV').setStyle({ width: '18px', height: '18px', overflow: 'hidden', zoom: 1, cursor: 'pointer' }).addClass("colorPicker").setStyle(this.ne.getIcon(buttonName, options)).appendTo(this.border);
		} else {
...
var nicEditorSelect = bkClass.extend({
	...
	construct : function(e,buttonName,options,nicEditor) {
		...
		this.contain = new bkElement('div').setStyle({width: '160px', height : '20px', cursor : 'pointer', overflow: 'hidden'}).addClass('selectContain').addEvent('click',this.toggle.closure(this)).appendTo(this.margin);
		this.items = new bkElement('div').setStyle({overflow : 'hidden', zoom : 1, border: '1px solid #ccc', paddingLeft : '3px', backgroundColor : '#fff'}).appendTo(this.contain);
		this.control = new bkElement('div').setStyle({overflow : 'hidden', 'float' : 'right', height: '18px', width : '16px'}).addClass('selectControl').setStyle(this.ne.getIcon('arrow',options)).appendTo(this.items);
		this.txt = new bkElement('div').setStyle({overflow : 'hidden', 'float' : 'left', width : '136px', height : '14px', marginTop : '1px', fontSize : '12px'}).addClass('selectTxt').appendTo(this.items);
	...
	open : function() {
	    this.pane = new nicEditorPane(this.items, this.ne, {
	        width: '158px', maxHeight: '225px', padding: '0px', borderTop: 0, borderLeft: '1px solid #ccc',
	        borderRight: '1px solid #ccc', borderBottom: '0px', backgroundColor: '#fff', overflowY: 'scroll'
	    });
		
		for(var i=0;i<this.selOptions.length;i++) {
			var opt = this.selOptions[i];
			var itmContain = new bkElement('div').setStyle({ borderBottom : '1px solid #ccc', width: '140px', textAlign : 'left', cursor : 'pointer'});
...
var nicEditorFontSizeSelect = nicEditorSelect.extend({
	sel : {1 : '8&nbsp;(8px)', 2 : '10&nbsp;(10px)', 3 : '12&nbsp;(12px)', 4 : '14&nbsp;(14px)', 5 : '18&nbsp;(18px)', 6 : '26&nbsp;(26px)'},
	init : function() {
...
var nicEditorFontFamilySelect = nicEditorSelect.extend({
    sel: {
        ...
        'verdana': 'Verdana',
        "'Enconomica', sans-serif": 'Economica',
...
var nicColorOptions = {
	buttons : {
		'forecolor' : {name : __('Change Text Color'), type : 'nicEditorColorButtonCustom', noClose : true},
		'bgcolor' : {name : __('Change Background Color'), type : 'nicEditorBgColorButton', noClose : true}
	}
};
/* END CONFIG */
var nicEditorColorButtonCustom = nicEditorAdvancedButton.extend({
    addPane: function () {
        var btn = this;
        var colorItems = new bkElement('DIV').addClass("colorPicker");
        var selection = window.getSelection();
        var range = selection.getRangeAt(0);
        
        var cp = $(colorItems).ColorPicker({
            color: '#0000ff',
            onShow: function (colpkr) {
                $(colpkr).fadeIn(500).position({
                    my: "center bottom",
                    at: "center top",
                    of: $(".nicEdit-pane")[0]
                });
            ...
            onSubmit: function (hsb, hex, rgb) {
                btn.colorSelect(hex, range);
                $(".colorpicker").hide();
            }
        });
        $(colorItems).trigger("click");
        this.pane.append(colorItems.noSelect())

    },
    colorSelect: function (c, range) {

        var selection = Utils.Selection;
        selection.clear();
        
        selection.add(Utils.Ranges.selectNode(range.commonAncestorContainer));
        var color = "#" + c;
...
var nicEditorBgColorButton = nicEditorColorButtonCustom.extend({
	colorSelect : function(c, range) {
	    var selection = Utils.Selection;
	    selection.clear();
